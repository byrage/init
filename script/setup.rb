class Setup
  ...
  def self.add_repository(ppa, continue=@skip)
  ...
  def self.apt_install(package, continue=@skip)
    puts `apt-get -y install #{package} #{continue}`
  end

  def self.apt_fix_broken_install(continue=@skip)
    puts `apt-get -f install #{continue}`
  end

  def self.wget_download(fileName, sourceUrl, continue=@skip)
    puts `wget -O #{fileName} #{sourceUrl} #{continue}`
  end

  def self.wget_run(sourceUrl, continue=@skip)
    puts `wget -O - #{sourceUrl} | bash #{continue}`
  end

  def self.dpkg_install(debName, continue=@skip)
  ...
  def self.export_in_profile(homeName, content, continue=@skip)
    if ENV["#{homeName}"] == nil
      puts `echo 'export #{homeName}=#{content}' >> #{@profile} #{continue}`
    end
  end

  def self.source_profile
